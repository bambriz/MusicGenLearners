...
num_epochs = 2000 #The number of training epochs that we are going to run. For each epoch we go through the entire data set.
batch_size = 100 #The number of training examples that we are going to send through the RBM at a time.
lr         = tf.constant(0.007, tf.float32) #The learning rate of our model
...
with tf.Session() as sess:
    ...
    for epoch in tqdm(range(num_epochs)):
        for song in songs:
            ...
            song = np.array(song)
            song = song[:(song.shape[0]//num_timesteps)*num_timesteps]
            song = np.reshape(song, [song.shape[0]//num_timesteps, song.shape[1]*num_timesteps])
            #Train the RBM on batch_size examples at a time
            for i in range(1, len(song), batch_size):
                tr_x = song[i:i+batch_size]
                sess.run(updt, feed_dict={x: tr_x//2})
